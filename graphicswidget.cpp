...
void GraphicsWidget::updatePlotterVisibility()
{
    ...
    {
        updatePlotter();
        static const QString s(tr("Ð¡Ð¿Ñ€ÑÑ‚Ð°Ñ‚ÑŒ Ð³Ñ€Ð°Ñ„Ð¸Ðº"));
        m_ui->makeChart->setText(s);
        m_chart->show();
    }
    else
    {
        static const QString s(tr("ÐŸÐ¾ÑÑ‚Ñ€Ð¾Ð¸Ñ‚ÑŒ Ð³Ñ€Ð°Ñ„Ð¸Ðº"));
        m_ui->makeChart->setText(s);
        m_chart->hide();
    }
}
...
{  
    ...
    m_chart->plotLayout()->addElement(0, 0, new QCPPlotTitle(m_chart, tr("Òåìïåðàòóðà")));

    m_chart->xAxis->setLabel("Ðàññòîÿíèå");
    m_chart->yAxis->setLabel("Òåìïåðàòóðà");
